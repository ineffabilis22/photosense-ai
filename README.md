...
PhotoSense AI 是一个面向摄影学习者的作品分析工具。用户选择摄影媒介、题材和点评深度，上传照片后获得构图、光线、色彩、叙事、技术完成度以及后期和下次拍摄建议。
...
`npm run check` 会依次完成测试源码类型检查、21 项自动测试和生产构建。自动测试包含 DOM 交互闭环以及本地模拟 OpenAI-compatible 供应商的端到端请求，不会调用或消耗真实 API Key。
