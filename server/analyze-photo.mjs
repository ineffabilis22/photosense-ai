...
const PROVIDER_TIMEOUT_MS = readBoundedNumber(process.env, 'PROVIDER_TIMEOUT_MS', 60_000, { min: 5_000, max: 180_000, integer: true });
const OPENAI_RELAY_TIMEOUT_MS = readBoundedNumber(process.env, 'OPENAI_RELAY_TIMEOUT_MS', 90_000, { min: 5_000, max: 180_000, integer: true });
const OPENAI_RELAY_MAX_TOKENS = readBoundedNumber(process.env, 'OPENAI_RELAY_MAX_TOKENS', 3_000, { min: 500, max: 8_000, integer: true });
...
const MAX_EXPORT_SNAPSHOTS = 20;
const MAX_BASE_EVALUATION_CACHE_ENTRIES = 50;
...
function normalizeSkillLevel(value) {
  return value === '进阶水平' || value === '进阶' || value === '高级' ? '进阶水平' : DEFAULT_SKILL_LEVEL;
}
...
function getReviewContext(medium = '数码摄影', genre = '街头摄影', skillLevel = DEFAULT_SKILL_LEVEL) {
  ...
  return {
    ...
    levelFocus: levelEvaluationFocus[skillLevel] || levelEvaluationFocus[DEFAULT_SKILL_LEVEL],
...
async function renderPreviewWithTimeout(input) {
  let timeoutId;
  const timeout = new Promise((_, reject) => {
    timeoutId = setTimeout(() => {
      const error = new Error('生成后期预览超时，请稍后重试。');
  ...
  try {
    return await Promise.race([renderPreviewImage(input), timeout]);
  } finally {
...
const hobbyistLanguageReplacements = [
  [' EV', ''],
  ['动态范围', '亮暗细节范围'],
  ['主体分离', '主体与背景的区分'],
  ['边缘管理', '画面边缘整理'],
...
function simplifyHobbyistValue(value) {
...
function normalizePhotoSpecific(value, fallback) {
  return {
    strength: sanitizeUserFacingText(value?.strength, fallback.strength),
    priorityIssue: sanitizeUserFacingText(value?.priorityIssue, fallback.priorityIssue),
    affectedArea: sanitizeUserFacingText(value?.affectedArea, fallback.affectedArea),
    nextAction: sanitizeUserFacingText(value?.nextAction, fallback.nextAction),
    crop: {
      ratio: sanitizeUserFacingText(value?.crop?.ratio, fallback.crop.ratio),
      direction: sanitizeUserFacingText(value?.crop?.direction, fallback.crop.direction),
      rationale: sanitizeUserFacingText(value?.crop?.rationale, fallback.crop.rationale),
    },
  };
}

function normalizeScoreReasons(value, fallback) {
  return scoreNames.reduce((result, name) => {
    result[name] = sanitizeUserFacingText(value?.[name], fallback[name]);
    return result;
  }, {});
}
...
function normalizeGenreAssessment(value) {
  ...
  const reason = typeof value?.reason === 'string' ? value.reason.trim().slice(0, 160) : '';

  if (!detectedGenre || !Number.isFinite(rawConfidence) || !reason) return undefined;
...
function createReportPrompt({ medium = '数码摄影', genre = '街头摄影', skillLevel = DEFAULT_SKILL_LEVEL, fileName = '', workTitle = '', title = '', toneProfile }) {
  ...
  const measuredAdjustments = toneProfile?.adjustments ?? {
  ...
  const toneMeasurement = toneProfile
    ? `- 亮度 P05/P50/P95：${Math.round(toneProfile.metrics.p05 * 100)} / ${Math.round(toneProfile.metrics.p50 * 100)} / ${Math.round(toneProfile.metrics.p95 * 100)}
- 暗部/高光剪切比例：${(toneProfile.metrics.shadowClipRatio * 100).toFixed(1)}% / ${(toneProfile.metrics.highlightClipRatio * 100).toFixed(1)}%
- 有效明暗跨度：${Math.round(toneProfile.metrics.dynamicRange * 100)}
- 后端测量得到的全局参数：${JSON.stringify(measuredAdjustments.global)}`
    : '- 未获得可靠影调测量；previewAdjustments 必须保持中性。';
...
输出 JSON 结构如下，字段名必须一致：
{
  ...
  "previewAdjustments": {
    "crop": {"ratio": "original", "anchorX": 0.5, "anchorY": 0.5},
    "global": ${JSON.stringify(measuredAdjustments.global)}
  },
  "photoSpecific": {
    "strength": "这张照片最值得保留的具体优点",
    "priorityIssue": "最优先解决的一个可见问题",
    "affectedArea": "问题所在的可见画面区域",
    "nextAction": "可以立即执行的一步",
    "crop": {"ratio": "比例或不建议裁剪", "direction": "裁剪方向", "rationale": "为什么这样裁剪"}
  },
  "scoreReasons": {
    "构图": "构图评分依据",
    "光线": "光线评分依据",
...
async function createRelayReport({ imageDataUrl, medium, genre, skillLevel, fileName, workTitle, title, toneProfile }) {
...
async function createAnthropicRelayReport({ imageDataUrl, medium, genre, skillLevel, fileName, workTitle, title, toneProfile }) {
...
function applyMeasuredToneProfile(report, toneProfile) {
  const adjustments = toneProfile.adjustments;
  ...
  return {
    ...
    recipe: {
      ...report.recipe,
      exposure: formatMeasuredValue(global.exposureEv, ' EV'),
      contrast: formatMeasuredValue(global.contrast),
      highlights: formatMeasuredValue(global.highlights),
      shadows: formatMeasuredValue(global.shadows),
      temperature: formatMeasuredValue(global.temperature),
      cropRatio: '保持原比例',
    },
...
async function createPhotoReport({ imageDataUrl, medium = '数码摄影', genre = '街头摄影', skillLevel = DEFAULT_SKILL_LEVEL, fileName = '', workTitle = '', title = '' }) {
  ...
  const stableBaseEvaluation = getStableBaseEvaluation({
    imageDataUrl,
    medium,
    genre,
  ...
  return normalizedSkillLevel === '爱好者水平'
    ? simplifyHobbyistValue(finalReport)
    : finalReport;
}
...
async function saveReportHistoryExport(body) {
  const records = Array.isArray(body?.records) ? body.records.slice(0, MAX_HISTORY_RECORDS) : [];
  ...
  const snapshotFiles = (await fs.readdir(EXPORTS_DIR))
    .filter((fileName) => /^photosense_reports_export_.*\.json$/.test(fileName))
    .sort()
    .reverse();
  await Promise.all(snapshotFiles.slice(MAX_EXPORT_SNAPSHOTS).map((fileName) => fs.unlink(path.join(EXPORTS_DIR, fileName))));
...
const server = http.createServer(async (request, response) => {
  ...
  if (requestUrl.pathname === '/api/health' && (request.method === 'GET' || request.method === 'HEAD')) {
    ...
    response.end(request.method === 'HEAD' ? undefined : healthPayload);
    return;
  }
  ...
  if (requestUrl.pathname === '/api/save-report-history') {
    if (!HISTORY_EXPORT_ENABLED) {
      sendJson(response, 403, { ok: false, error: '报告历史导出仅在明确启用的本地开发环境中可用。' });
      return;
    }
  ...
  if (requestUrl.pathname === '/api/render-preview') {
    try {
      const body = await readJsonBody(request);
      ...
      console.error('[PhotoSense AI] preview render failed:', error);
