...
const OPENAI_RELAY_TEMPERATURE = readBoundedNumber(process.env, 'OPENAI_RELAY_TEMPERATURE', 0.45, { min: 0, max: 1 });
...
10. scoreReasons 必须分别解释五项分数为什么是当前数值，不能只是重复分数等级。
...
必须输出这些顶层字段：overall, verdict, reviewContext, scores, scoreReasons, photoSpecific, composition, lighting, colour, storytelling, technical, suggestions, postProcessing, nextShooting。recipe 可省略。
